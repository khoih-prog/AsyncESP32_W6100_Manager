/****************************************************************************************************************************
  ...
  4. Khoi Hoang          (https://github.com/khoih-prog/AsyncESP32_W6100_manager)
...
#if !( defined(ESP32) )
  #error This code is intended to run on the (ESP32 + W6100) platform! Please check your Tools->Board setting.
#endif
...
void setup()
{
  ...
  Serial.println(ASYNC_ESP32_W6100_MANAGER_VERSION);
...
#if ( USING_ESP32_S2 || USING_ESP32_C3 )
  AsyncESP32_W6100_Manager AsyncESP32_W6100_manager(&webServer, NULL, "AsyncConfigOnSwitch");
#else
  AsyncDNSServer dnsServer;

  AsyncESP32_W6100_Manager AsyncESP32_W6100_manager(&webServer, &dnsServer, "AsyncConfigOnSwitch");
#endif

  AsyncESP32_W6100_manager.setDebugOutput(true);

#if !USE_DHCP_IP
  // Set (static IP, Gateway, Subnetmask, DNS1 and DNS2) or (IP, Gateway, Subnetmask)
  AsyncESP32_W6100_manager.setSTAStaticIPConfig(EthSTA_IPconfig);
#endif
  ...
  if (initialConfig)
  {
    ...
    //AsyncESP32_W6100_manager.setConfigPortalTimeout(600);
